...
if (isset($_POST['modifi'])) {
    ...
    $naiss = $_POST['dates_naissance'] ;
    $entre = $_POST['dates_entrer'] ;
    $telephone = $_POST['adresse'] ;
    $service = $_POST['service'];
    $id = $_POST['id'];
    $con = mysql_connect("localhost", "root", "") or die(mysql_error()) ;
    $db = mysql_select_db("lsdi") or die(mysql_error()) ;
    $requete = " UPDATE techniciens SET matricule ='$code', nom='$nom', prenom='$prenom',specialiter='$specia', dates_naissance='$naiss', dates_entrer='$entre', adresse='$telephone', etat_service='$service' WHERE id='$id' " ;
    $sql = mysql_query($requete, $con) or die(mysql_error()) ;
    header("location: listetech.PHP") ;
