...
// code d'enregistrement
   ...
   if (isset($_POST['enregistrer'])){
    if(isset($_POST['code']) || !empty($_POST['nom']) || !empty($_POST['prenom']) || !empty($_POST['telephone']) || !empty($_POST['fax']) || !empty($_POST['email']) || !empty($_POST['compa']) || !empty($_POST['local'])){
      ...
      $req = " SELECT code FROM client WHERE etat='on' ";
      $sql = mysql_query($req, $con) or die(mysql_error()) ;
      $resultat = mysql_fetch_assoc($sql) ;

        if ( $resultat['code'] == $code){
          ?>
            <script>
                alert('LE CODE D\'IDENTIFIANT ENTRER EXISTE DEJA VEUILLEZ LE CHANGER ET RELANCER VOTRE ENREGISTREMENT');
                window.location.href ="enregistrementclient.php"
            
            </script>
          <?php

         // echo 'CE CODE D\'IDENTIFIANT EXISTE DEJA VEUILLEZ LE CHANGER';
         // echo '<a href="enregistrementclient.php"><button  class="btn btn-primary" type="reset">Compris</button>	 </a>';
        }else{
...
// enregistrement du traitement
   if(isset($_POST['enregistrer'])){
    ...
    $description = "Mr/Mme"." ".$_SESSION['login']." a effectuer  l'enregistrement de Mr/Mme"." ".$nom." ";
...
<!--section du formulaire -->
                ...
                <div class="form-group">
                 ...
                 <div>
                  ...
                  <div>
                    ...
                    <div class="col-sm-10">
                     <input id="telephone" maxlength="12" minlength="1" name='telephone' type="number" class="form-control" placeholder="Téléphone du client" autocomplete="off" required="" /><br>
                    </div> 
                  </div>
                  <div>
  ...
  <script>
      $("#submit").click(function(){
         ...
         if(code == '' || nom == '' || prenom == '' || telephone == '' || client == '' || email == '' || compa == '' || local ==''){
