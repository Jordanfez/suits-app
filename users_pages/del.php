...
if (isset($_POST['modif'])) {
    ...
    $fax = $_POST['fax'] ;
    $email = $_POST['email'] ;
    $compa = $_POST['compagnie'] ;
    $local = $_POST['localisation'] ;
    $id = $_POST['id'];
    $con = mysql_connect("localhost", "root", "") or die(mysql_error()) ;
    $db = mysql_select_db("lsdi") or die(mysql_error()) ;
    $requete = " UPDATE client SET code ='$code', nom_client='$nom', prenom='$prenom', telephone='$telephone', fax='$fax', email='$email', compagnie='$compa', localisation='$local' WHERE id='$id' " ;
    $sql = mysql_query($requete, $con) or die(mysql_error()) ;
    header("location: listeclient.php") ;
